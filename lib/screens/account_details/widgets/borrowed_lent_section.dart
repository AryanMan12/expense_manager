...
class BorrowedLentSection extends StatelessWidget {
  final String title;
  final IconData icon;
  final double total;
  ...
  Widget build(BuildContext context) {
    final sortedTxns = [...transactions]
      ..sort(
        (a, b) => DateTime.parse(
          a.expenseDate ?? '',
        ).compareTo(DateTime.parse(b.expenseDate ?? '')),
      );

    final sumOfTransactions = sortedTxns.fold<double>(
      0.0,
      (sum, t) => sum + (t.amount ?? 0.0),
    );
    final remaining = (total - sumOfTransactions).clamp(0, double.infinity);
  ...
  List<Widget> _buildTransactionList(
    ...
    double runningRemaining = total;

    return txns.map((t) {
      final amount = t.amount ?? 0;
      runningRemaining = (runningRemaining - amount).clamp(0, double.infinity);
      ...
      return ListTile(
        ...
        trailing: Column(
          ...
          children: [
            ...
            Text(
              "Rem: ₹${runningRemaining.toStringAsFixed(2)}",
              style: TextStyle(
                fontSize: 11,
                color: runningRemaining > 0 ? Colors.orange : Colors.green,
